package org.remoteHome.stk500_v1;
	...
	private boolean spamSync() {
		...
		for (int i = 0; i < 500; i++) {
			if (reader.getState() == EReaderState.TIMEOUT_OCCURRED) {
				if (!waitForReaderStateActivated(50)) {
...
//			memtype = (byte)'E';
	...
	private boolean uploadFile(int bytesToLoad, boolean write) {
		...
		logger.logcat("Programming: " + getProgress() + " %", "d");
		...
		while (hexPosition < hexParser.getDataSize()) {
			...
			if(write) {
				...
				if (programPage(useFlash, tempArray)) {
					...
					logger.logcat("Programming: " + getProgress() + " % " +
				...
				if(Arrays.equals(rdata, tempArray)) {
					...
					logger.logcat("Verifying: " + getProgress() + " % ", "d");
				}
				else {
	...
	IOException {
		...
		waitForReaderStateActivated(50);
